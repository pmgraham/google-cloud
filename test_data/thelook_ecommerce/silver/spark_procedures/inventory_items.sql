CREATE OR REPLACE PROCEDURE `biglake-iceberg-datalake.silver.transform_inventory_items`()
...
df = read_bronze("inventory_items")
df = dedup(df, "id")

# ── Transformations ──
# Cast id, product_id, product_distribution_center_id from FLOAT to LongType
df = df.withColumn("id", safe_cast_to_long("id"))
df = df.withColumn("product_id", safe_cast_to_long("product_id"))
df = df.withColumn("product_distribution_center_id", safe_cast_to_long("product_distribution_center_id"))

# Strings: product_category, product_name, product_brand, product_department -> initcap
df = df.withColumn("product_category", clean_string_initcap("product_category"))
df = df.withColumn("product_name", clean_string_initcap("product_name"))
df = df.withColumn("product_brand", clean_string_initcap("product_brand"))
df = df.withColumn("product_department", clean_string_initcap("product_department"))

# product_sku -> UPPER
df = df.withColumn("product_sku", clean_string_upper("product_sku"))

# Timestamps: created_at, sold_at STRING -> TIMESTAMP
df = df.withColumn("created_at", parse_multi_format_timestamp("created_at"))
df = df.withColumn("sold_at", parse_multi_format_timestamp("sold_at"))

# Add silver_loaded_at timestamp
df = df.withColumn("silver_loaded_at", F.current_timestamp())

# ── Select final columns (drop cost_value_type, product_retail_price_value_type) ──
df = df.select(
    "id",
    "product_id",
    "created_at",
...
# ── Write to silver ──
write_silver(df, "inventory_items")

# ── Export Iceberg metadata to BigLake Metastore ──
from google.cloud import bigquery
bq_client = bigquery.Client(project=PROJECT)
bq_client.query("EXPORT TABLE METADATA FROM `biglake-iceberg-datalake.silver.inventory_items`").result()
""";
