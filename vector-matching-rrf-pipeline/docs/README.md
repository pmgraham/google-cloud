...
When a user runs the full pipeline orchestrator (`uv run python pipeline/run.py --reset`) and follows up with the agent (`uv run python agent/agent.py`), they will see the data transform from raw configuration into a fully evaluated mapping graph.
...
For the exceptions picked up by the ADK Agent, users can query the `agent_decisions` table to see the exact logic used to resolve the conflict. For example:
