...
materials = {
    "Fasteners": [
        (
            "Stainless Steel",
            "304",
            ["SS", "Stainless", "18-8", "A2-70", "Type 304", "304 SS"],
        ),
        (
            "Stainless Steel",
            "316",
            ["SS", "Stainless", "316", "A4-80", "Type 316", "316 SS"],
        ),
        (
            "Carbon Steel",
            "Grade 5",
            ["CS", "Carbon Steel", "Plain Steel", "Gr 5", "Grade 5"],
        ),
        (
            "Carbon Steel",
            "Grade 8",
            ["CS", "Carbon Steel", "Black Oxide", "Gr 8", "Grade 8"],
        ),
...
def generate_canonical_parts(num_parts):
    ...
    for _ in range(num_parts):
        ...
        # Assign material
        mat_options = (
            materials["Fasteners"] if category == "Fasteners" else materials["General"]
        )
        ...
        if category == "Fasteners" and part_type in [
            "Hex Bolt",
            "Socket Head Cap Screw",
            "Flat Head Screw",
            "Set Screw",
...
for i in range(30):
    ...
    for s in sources:
        truth_table.append(
            {"canonical_part_id": p["canonical_part_id"], "source": s, "part_data": p}
        )
...
def get_source_data(record, counter):
    ...
    # 2. Source1 (Fastenal-style: FAS-PN, moderate desc, heavy abbrev)
        ...
        part_description = (
            f"{format_abbreviation(p['part_type'])} {p['dim']} {mat_alias}".upper()
        )
        material = mat_alias
        dimensions = p["dim"]
        category = p["category"]
    ...
    # 4. Source3 (Legacy MRO: Short codes, messy, nulls)
        ...
        if random.random() < 0.2:
            type_str = (
                type_str.replace("bolt", "blt")
                .replace("screw", "scrw")
                .replace("stainless", "stainles")
            )

        dim_str = (
            p["dim"].lower().replace("x", " ").replace("-", " ").replace("  ", " ")
        )
...
truth_df = df.select(
    ["canonical_part_id", "canonical_description", "source", "part_number"]
)
...
for s in sources:
    source_df = df.filter(pl.col("source") == s).select(
        ["part_number", "part_description", "material", "dimensions", "category"]
    )
...
for s in sources:
    print(
        f" - raw_{s.lower()}_parts.csv: {df.filter(pl.col('source') == s).height} rows"
    )
